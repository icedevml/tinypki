...
app_logger = structlog.stdlib.get_logger(config.LOG_NAME)
access_logger = structlog.stdlib.get_logger(config.LOG_ACCESS_NAME)
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        try:
            ...
            app_logger.exception(
                ...
                exc_info=e,
                stack_info=config.LOG_INCLUDE_STACK,
            )
            ...
            access_logger.info(
                f"""{client_host}:{client_port} - "{http_method} {scope["path"]} HTTP/{http_version}" {info.get("status_code", 500)}""",
                http={
                    "url": str(url),
